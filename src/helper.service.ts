...
export class HelperService {
    ...
    constructor() {
        ...
        if (this.config.settings.logging) {
            this.log_file = fs.createWriteStream(__dirname + '/logs/debug.' + dateFormat(now, "yyyy_mm_dd_H_MM_ss.l") + '.log', { flags: 'w' });
            this.log_file_live = fs.createWriteStream(__dirname + '/debug-live.log', { flags: 'w' });
        }
    ...
    public log(d, color = '\x1b[0m') {
        if (this.config.settings.logging) {
            this.log_file.write("[" + dateFormat(new Date(), "yyyy-mm-dd H:MM:ss.l") + "] " + util.format(d) + '\n');
            this.log_file_live.write("[" + dateFormat(new Date(), "yyyy-mm-dd H:MM:ss.l") + "] " + util.format(d) + '\n');
        }
        console.log(color + "[" + dateFormat(new Date(), "yyyy-mm-dd H:MM:ss.l") + "] " + util.format(d), this.colors.FgWhite);
    };
